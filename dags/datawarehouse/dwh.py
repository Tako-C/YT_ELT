...
def staging_table():
    ...
    try:
        ...
        YT_data = load_data()
        logger.info("===== DEBUG YT DATA =====")
        logger.info(f"type(YT_data): {type(YT_data)}")
        logger.info(f"len(YT_data): {len(YT_data)}")
        logger.info(f"YT_data preview: {YT_data[:1] if YT_data else 'EMPTY'}")
        logger.info("========================")
